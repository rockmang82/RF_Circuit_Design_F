...
from PyQt5.QtWidgets import (
    ...
    QFileDialog, QMenu, QProgressBar, QScrollArea, QFrame
)
...
class Component:
    ...
    def __init__(self, comp_type, x, y):
        ...
        self.tolerance = 1.0        # Tolerance (%, default 1%)
...
class CircuitCanvas(QWidget):
    ...
    def _open_comp_dialog(self, comp):
        ...
        tol_edit = QLineEdit(str(getattr(comp, 'tolerance', 1.0)))
        tol_edit.setValidator(QDoubleValidator(0.0, 100.0, 4))
        tol_edit.setFixedWidth(80)
        tol_row.addWidget(tol_edit)
        tol_row.addWidget(QLabel('%'))
        tol_row.addStretch()
        dlg_layout.addLayout(tol_row)
        ...
        # Tolerance 적용
        tol_str = tol_edit.text().strip()
        if tol_str:
            try:
                comp.tolerance = float(tol_str)
            except ValueError:
                pass
...
class SensitivityWindow(QDialog):
    ...
    def __init__(self, f_mhz, z_nominal, results,
                 components=None, wires=None,
                 f_start_mhz=0.1, f_stop_mhz=100.0,
                 parent=None):
        super().__init__(parent)
        ...
        self._wires = wires or []
        self._f_start_mhz = f_start_mhz
        self._f_stop_mhz = f_stop_mhz
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        left_vbox.addWidget(table, stretch=1)

        bottom_btn_row = QHBoxLayout()
        bottom_btn_row.setSpacing(4)

        sweep_btn = QPushButton('Sensitivity Sweep')
        sweep_btn.setFixedHeight(28)
        ...
        bottom_btn_row.addWidget(sweep_btn)

        phase2_btn = QPushButton('Phase2 Analysis')
        phase2_btn.setFixedHeight(28)
        phase2_btn.setStyleSheet(
            'background-color: #4A90D9; color: white; font-size: 10pt; border-radius: 3px;'
        )
        phase2_btn.clicked.connect(self._open_phase2)
        bottom_btn_row.addWidget(phase2_btn)

        left_vbox.addLayout(bottom_btn_row)

        content.addWidget(left_panel, stretch=45)
    ...
    def _open_sweep(self):
        """Sensitivity Sweep: 주파수 범위 전체에 걸쳐 각 소자의 Norm.S 계산 후 그래프 출력"""
        import copy

        if not self._components:
            QMessageBox.warning(self, '오류', '소자 정보가 없습니다.')
            return

        f_start = self._f_start_mhz
        f_stop = self._f_stop_mhz
        n_pts = int(round((f_stop - f_start) / 0.1)) + 1
        freqs_mhz = np.linspace(f_start, f_stop, n_pts)

        comps = copy.deepcopy(self._components)
        wires = self._wires
        eps = 0.01

        valid_comps = [c for c in comps if c.value is not None and c.value != 0]
        if not valid_comps:
            QMessageBox.warning(self, '오류', '민감도를 계산할 소자가 없습니다.')
            return

        comp_names = [
            c.name if c.name else f'{c.type}{i+1}'
            for i, c in enumerate(valid_comps)
        ]
        norm_arrays = {name: np.zeros(n_pts) for name in comp_names}

        progress = QProgressDialog('Sensitivity Sweep 계산 중...', '취소', 0, n_pts, self)
        progress.setWindowTitle('계산 중')
        progress.setWindowModality(Qt.ApplicationModal)
        progress.setMinimumDuration(0)
        progress.setValue(0)
        progress.show()

        for idx, f_mhz_val in enumerate(freqs_mhz):
            ...
            f_hz = f_mhz_val * 1e6
            Z0 = mna_solve_one(comps, wires, f_hz)
            abs_Z0 = abs(Z0) if Z0 is not None else 0.0

            for comp, name in zip(valid_comps, comp_names):
                xi = comp.value
                comp.value = xi * (1.0 + eps)
                Z_plus = mna_solve_one(comps, wires, f_hz)
                comp.value = xi * (1.0 - eps)
                Z_minus = mna_solve_one(comps, wires, f_hz)
                comp.value = xi  # 복원

                if Z_plus is None or Z_minus is None or abs_Z0 == 0:
                    norm_arrays[name][idx] = 0.0
                else:
                    abs_sens = (abs(Z_plus) - abs(Z_minus)) / (2.0 * xi * eps)
                    norm_arrays[name][idx] = abs_sens * (xi / abs_Z0)

        progress.setValue(n_pts)

        sweep_list = [
            {'name': name, 'norm_sens': norm_arrays[name]}
            for name in comp_names
        ]
        win = SensitivitySweepWindow(freqs_mhz, sweep_list, self.f_mhz, self)
        win.show()

    def _open_phase2(self):
        """Phase2 Analysis: Tolerance 기반 불확실도 예산 분석"""
        if not self._components:
            QMessageBox.warning(self, '오류', '소자 정보가 없습니다.')
            return
        ...
        # 소자 이름 → tolerance 매핑
        tol_map = {
            c.name: getattr(c, 'tolerance', 1.0)
            for c in self._components if c.name
        }

        phase2_results = []
        for r in self.results:
            tol = tol_map.get(r['name'], 1.0)
            delta_xi = r['value'] * (tol / 100.0)
            delta_z = abs(r['abs_sens']) * delta_xi
            phase2_results.append(dict(r, tolerance=tol, delta_z=delta_z))

        rss = math.sqrt(sum(r['delta_z'] ** 2 for r in phase2_results))
        worst_case = sum(r['delta_z'] for r in phase2_results)

        win = Phase2Window(
            self.f_mhz, self.z_nominal, phase2_results, rss, worst_case, self
        )
        win.show()


# ──────────────────────────────────────────────
# Phase2 Analysis 결과 윈도우
# ──────────────────────────────────────────────

class Phase2Window(QDialog):
    """Phase2: Tolerance 기반 ΔZ_i, RSS, Worst-Case, Budget Breakdown"""

    def __init__(self, f_mhz, z_nominal, phase2_results, rss, worst_case, parent=None):
        super().__init__(parent)
        self.setWindowTitle('Phase2 Analysis — Uncertainty Budget')
        self.setWindowFlags(
            (self.windowFlags() | Qt.Window)
            & ~Qt.WindowContextHelpButtonHint
        )
    ...
    # ── 단위 포맷 헬퍼 ──
    def _fmt_ohm(self, val):
        a = abs(val)
        if a < 1e-6:
            return f'{val * 1e9:.3f} nΩ'
        elif a < 1e-3:
            return f'{val * 1e6:.3f} μΩ'
        elif a < 1.0:
            return f'{val * 1e3:.3f} mΩ'
        else:
            return f'{val:.3f} Ω'
    ...
    def _build_ui(self):
        ...
        table = QTableWidget(n_rows, 7)
        table.setHorizontalHeaderLabels(
            ['Component', 'Value', 'Tol(%)', 'ΔZ_i(Ω)', 'ΔZ/ΔXi', 'Norm.S', 'Rank']
        )
        table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        table.setEditTriggers(QTableWidget.NoEditTriggers)
        table.setSelectionMode(QTableWidget.ExtendedSelection)
        ...
        highlight_count = 5 if n_rows > 5 else 3
        max_abs = abs(sorted_r[0]['norm_sens']) if sorted_r else 1.0
        if max_abs == 0:
            max_abs = 1.0
        ...
        right_vbox.setContentsMargins(8, 4, 8, 4)
        right_vbox.setSpacing(12)

        self._build_summary_section(right_vbox)
        self._build_budget_section(right_vbox)
    ...
    def _build_summary_section(self, parent_layout):
        ...
        rss_pct = self.rss / self.z_nominal * 100 if self.z_nominal != 0 else 0.0
        wc_pct = self.worst_case / self.z_nominal * 100 if self.z_nominal != 0 else 0.0

        cards_row.addWidget(self._make_card(
            'RSS (통계적)', self._fmt_ohm(self.rss),
            f'≈ ±{rss_pct:.3f}% of |Z₀|', '#1565C0'
        ))
        cards_row.addWidget(self._make_card(
            'WORST-CASE', self._fmt_ohm(self.worst_case),
            f'≈ ±{wc_pct:.3f}% of |Z₀|', '#C62828'
        ))
        vbox.addLayout(cards_row)
        parent_layout.addWidget(frame)

    def _make_card(self, label_text, value_text, sub_text, val_color):
        card = QFrame()
        card.setStyleSheet(
            'QFrame { background-color: white; border-radius: 8px; border: 1px solid #DEE2E6; }'
        )
        ...
        lbl.setStyleSheet('font-size: 9pt; color: #666; letter-spacing: 1px; border: none;')
        lbl.setAlignment(Qt.AlignCenter)

        val = QLabel(value_text)
        val.setStyleSheet(
            f'font-size: 18pt; font-weight: bold; color: {val_color}; border: none;'
        )
        val.setAlignment(Qt.AlignCenter)

        sub = QLabel(sub_text)
        sub.setStyleSheet('font-size: 9pt; color: #888; border: none;')
    ...
    def _build_budget_section(self, parent_layout):
        frame = QFrame()
        frame.setStyleSheet(
            'QFrame { background-color: #F8F9FA; border-radius: 8px; border: 1px solid #DEE2E6; }'
        )
        ...
        title = QLabel('불확실도 할당 (Budget Breakdown)')
        title.setStyleSheet(
            'font-size: 12pt; font-weight: bold; color: #1565C0; border: none;'
        )
        vbox.addWidget(title)

        sorted_by_dz = sorted(self.results, key=lambda r: -r['delta_z'])
        max_dz = sorted_by_dz[0]['delta_z'] if sorted_by_dz else 1.0
        if max_dz == 0:
            max_dz = 1.0
        total_wc = self.worst_case if self.worst_case != 0 else 1.0

        # tab10 색상: 소자 이름 알파벳 순서로 인덱스 배정
        tab10 = plt.cm.tab10.colors
        name_sorted = sorted(r['name'] for r in self.results)
        color_map = {
            name: '#{:02x}{:02x}{:02x}'.format(
                int(tab10[i % len(tab10)][0] * 255),
                int(tab10[i % len(tab10)][1] * 255),
                int(tab10[i % len(tab10)][2] * 255),
            )
            for i, name in enumerate(name_sorted)
        }

        for r in sorted_by_dz:
            color_hex = color_map[r['name']]
            pct_of_wc = r['delta_z'] / total_wc * 100

            # 이름 + 값 행
            row_layout = QHBoxLayout()
            row_layout.setContentsMargins(0, 0, 0, 0)
            name_lbl = QLabel(f'{r["name"]} (±{r["tolerance"]:.1f}%)')
            name_lbl.setStyleSheet(
                f'font-size: 10pt; font-weight: bold; color: {color_hex}; border: none;'
            )
            val_lbl = QLabel(f'{self._fmt_ohm(r["delta_z"])} ({pct_of_wc:.1f}%)')
            val_lbl.setStyleSheet('font-size: 10pt; color: #444; border: none;')
            ...
            bar.setValue(int(r['delta_z'] / max_dz * 10000))
    ...
    def _copy_selection_to_clipboard(self, table):
        ...
        for r in rows:
            lines.append('\t'.join(
                (table.item(r, c).text() if table.item(r, c) else '') for c in cols
            ))
        QApplication.clipboard().setText('\n'.join(lines))


# ──────────────────────────────────────────────
# Sensitivity Sweep 결과 윈도우
# ──────────────────────────────────────────────

class SensitivitySweepWindow(QDialog):
    """Sensitivity Sweep: 주파수별 각 소자의 Norm.S 그래프"""

    def __init__(self, freqs_mhz, sweep_list, f_marker_mhz, parent=None):
        """
        freqs_mhz    : np.ndarray — 주파수 배열 (MHz)
        sweep_list   : list of {'name': str, 'norm_sens': np.ndarray}
        f_marker_mhz : float — 원본 단일 주파수 분석 지점 (빨간 점선 마커)
        """
        super().__init__(parent)
    ...
    def _plot(self):
        ...
        colors = plt.cm.tab10.colors  # 10가지 구분 색상

        for i, entry in enumerate(self._sweep_list):
            color = colors[i % len(colors)]
            ax.plot(
                self._freqs, entry['norm_sens'],
                label=entry['name'], color=color, lw=1.5
            )

        # 단일 분석 주파수 마커 (빨간 점선)
        ax.axvline(
            self._f_marker, color='red', linestyle='--', lw=1.2,
            label=f'@ {self._f_marker:.3f} MHz', zorder=10
        )

        # y=0 기준선
        ax.axhline(0, color='#888888', lw=0.8, linestyle='-', zorder=1)

        ax.set_xlabel('Frequency (MHz)', fontsize=10)
        ax.set_ylabel('Normalized Sensitivity (ΔZ/Z per ΔXi/Xi)', fontsize=9)
        ax.set_title('Sensitivity Sweep', fontsize=11)
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # MC Analysis + Sensitivity Analysis 버튼 (창1 우측 상단)
        sens_row = QHBoxLayout()
        sens_row.addStretch()

        self.mc_btn = QPushButton('MC Analysis')
        self.mc_btn.setFixedHeight(28)
        self.mc_btn.setStyleSheet(
            'background-color: #4A90D9; color: white; font-size: 10pt; border-radius: 3px;'
        )
        self.mc_btn.clicked.connect(self._open_mc_analysis)
        sens_row.addWidget(self.mc_btn)
            ...
            'components': [
                {
                    ...
                    'tolerance': getattr(c, 'tolerance', 1.0),
                }
    ...
    def _on_load(self):
        ...
        for cd in data.get('components', []):
            ...
            comp.tolerance = cd.get('tolerance', 1.0)
    ...
    def _open_mc_analysis(self):
        """Monte Carlo Simulation 버튼 처리"""
        ...
        if not connected_comps:
            QMessageBox.warning(self, '안내', 'Monte Carlo 분석에는 값이 입력된 소자가 필요합니다')
            return

        try:
            from Circuit_MC_Simulation import open_mc_analysis
        except ImportError:
            QMessageBox.warning(self, '오류', 'Circuit_MC_Simulation.py 파일을 찾을 수 없습니다')
            return
    ...
    def _on_sensitivity_finished(self, f_mhz, z_nominal, results):
        ...
        try:
            f_start_mhz = float(self.freq_start.text())
            f_stop_mhz = float(self.freq_stop.text())
        except ValueError:
            f_start_mhz, f_stop_mhz = 0.1, 100.0
        win = SensitivityWindow(
            f_mhz, z_nominal, results,
            components=self.circuit_canvas.components,
            wires=self.circuit_canvas.wires,
